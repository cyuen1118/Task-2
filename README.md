...
Heap could be min-heap or max heap. For the min-heap, it is a complete binary tree. 
The value of the root node must be the smallest among all its descendant nodes and the same thing must be done for its left and right sub-tree(parent ≤ children). 
...
For the heap sort, it is a comparison-based sorting algorithm based on the Binary Heap data structure. The algorithm repeatedly finds the maximum (or minimum) element and swaps it with the last (or first) element.
To execute the heap sort, the algorithm will first visualize the array as a complete binary tree. Then it will build a Max Heap and sort the array by placing largest element at end of unsorted array. 
